import { countNumberFrequency } from '../src/index';

describe('countNumberFrequency', () => {
    ...
    it('handles unsorted input and returns sorted numbers', () => {
        const result = countNumberFrequency([5, 2, 4, 2]);
        expect(result.numbers.sort((a, b) => a - b)).toEqual([2, 3, 4, 5]);
        expect(result.frequencies).toEqual([2, 0, 1, 1]);
    });

    it('handles large range with missing numbers', () => {
        const result = countNumberFrequency([1, 10]);
